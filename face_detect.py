...
parser.add_argument('--save_folder', default='./dets/',
                    type=str, help='Dir to save txt results')
parser.add_argument('--cpu', action="store_true",
                    default=False, help='Use cpu inference')
parser.add_argument('--dataset_folder',
                    default='../advis/data/', type=str, help='dataset path')
...
if __name__ == '__main__':
    ...
    # testing dataset of historical adverts
    image_set = glob.glob(args.dataset_folder + '/**/*')  # load image files
    annotations = glob.glob('../advis/annotations/' +
                            '/*')  # load annotation xml
    ...
    for i in range(0, len(image_set)):
        ...
        if img_id in annotations_base:
            ...
            img_raw = cv2.imread(image_set[i], cv2.IMREAD_COLOR)
            ...
            img -= (104, 117, 123)
            # img -= (150, 170, 181) #using color means derived from data
            ...
            # --------------------------------------------------------------------
            save_name = args.save_folder + img_id + ".txt"
            ...
            with open(save_name, "w") as fd:
                ...
                # fd.write(file_name)
                # fd.write(bboxs_num)
                for box in bboxs:  # changed output to fit other output
                    xmin = int(box[0])
                    ymin = int(box[1])
                    xmax = int(box[2])
                    ymax = int(box[3])
                    ...
                    line = str('face') + " " + str(confidence) + " " + str(xmin) + \
                        " " + str(ymin) + " " + str(xmax) + \
                        " " + str(ymax) + " \n"
                    fd.write(line)
